...
Then upload that JSON file to IPFS or HTTPS, for example:
...
```bash
cd ainft-merchant-agent
python3 scripts/register_8004.py --uri ipfs://<bootstrap-cid>/registration.json
```
...
```bash
cd ainft-merchant-agent
python3 scripts/update_8004.py --agent-id <agent_id> --uri ipfs://<final-cid>/registration.json
```
